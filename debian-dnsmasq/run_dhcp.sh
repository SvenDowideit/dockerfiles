...
/home/sven/src/docker/un/docker-0.6.5-dev run -i -t  -d -name=dnsmasq svendowideit/dnsmasq /bin/bash 
#dnsmasq -d
...
#and the opposite..?
iptables -t nat -A PREROUTING -p all -d $DHCP_SERVER_IP -j dhcpserver
iptables -t nat -A OUTPUT -p all -d $DHCP_SERVER_IP -j dhcpserver

iptables -t nat -A dhcpserver -p all -j DNAT --to-destination $EXTERNAL_DHCP_SERVER_IP
#end opposite
